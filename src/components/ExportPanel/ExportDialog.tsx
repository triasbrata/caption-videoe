...
export function ExportDialog({
	...
	const handleChooseSaveLocation = async () => {
		...
		try {
			const handle = await (window as unknown as { showSaveFilePicker(opts?: { suggestedName?: string; types?: Array<{ description: string; accept: Record<string, string[]> }> }): Promise<FileHandle> }).showSaveFilePicker({
								...
								<div className="space-y-2">
									...
									>
										<div className="flex items-center space-x-2 min-w-0">
											...
											>
												{fileHandle
													? t("components.exportDialog.saveLocationChosen", { name: fileHandle.name })
													: t("components.exportDialog.chooseSaveLocation")}
											</span>
										</div>
										<button
