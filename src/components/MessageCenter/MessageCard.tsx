...
function formatRelativeTime(
	...
	if (diff < 60000) {
		...
		return t("components.messageCard.minutesAgo", {
			minutes: Math.floor(diff / 60000),
		});
	} else if (diff < 86400000) {
		return t("components.messageCard.hoursAgo", {
				...
				<div className="flex-shrink-0 mt-0.5">
					<Icon
						className={cn(
							"h-5 w-5",
							colors.icon,
							message.type === "processing" && "animate-spin",
						)}
					/>
				</div>
						...
						<div className="mt-3 space-y-2">
							<div className="flex justify-between items-center text-xs">
								<span className="text-muted-foreground capitalize">
									{message.progress.stage === "analyzing" &&
										t("components.messageCard.stageAnalyzing")}
									{message.progress.stage === "cutting" &&
										t("components.messageCard.stageCutting")}
									{message.progress.stage === "encoding" &&
										t("components.messageCard.stageEncoding")}
									{message.progress.stage === "complete" &&
										t("components.messageCard.stageComplete")}
									{message.progress.stage === "error" &&
										t("components.messageCard.stageError")}
								</span>
