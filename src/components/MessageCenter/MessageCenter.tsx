...
export function MessageCenter({ isOpen, onClose }: MessageCenterProps) {
  ...
  const filterOptions = useMemo(
    () => [
      ...
      {
        label: t("components.messageCenter.filterSuccess"),
        value: "success" as const,
      },
      {
        label: t("components.messageCenter.filterError"),
        value: "error" as const,
      },
      {
        label: t("components.messageCenter.filterWarning"),
        value: "warning" as const,
      },
      {
        label: t("components.messageCenter.filterInfo"),
        value: "info" as const,
      },
    ],
    [t]
    ...
    >
      ...
      >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
              ...
              <select
                value={filter}
                onChange={(e) =>
                  setFilter(e.target.value as MessageType | "all")
                }
        ...
        <div className="p-3 border-t text-xs text-muted-foreground text-center">
          {t("components.messageCenter.totalMessages", {
            count: messages.length,
          })}
          {unreadCount > 0 &&
            ` · ${t("components.messageCenter.unreadMessages", { count: unreadCount })}`}
        </div>
      </div>
    </div>
  );
