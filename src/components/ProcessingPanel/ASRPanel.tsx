...
export function ASRPanel({ className }: ASRPanelProps) {
  ...
  const hasTranscriptChunks = useHistoryStore(
    ...
    const handleProgress = (progress: ASRProgress) => {
      ...
      if (progress.status === "complete" && progress.result) {
        ...
        const duration = progress.time
          ? (progress.time / 1000).toFixed(1)
          : "0";
        showSuccess(
          t("components.asrPanel.asrCompleteMessage"),
          t("components.asrPanel.asrCompleteDetail", {
            count: chunkCount,
            duration,
          })
        );
      ...
      if (progress.status === "error") {
        ...
        showError(
          t("components.asrPanel.asrErrorMessage"),
          progress.error || t("components.asrPanel.unknownError")
        );
      }
      ...
      if (progress.status === "loading") {
        showInfo(
          t("components.asrPanel.loadingModel"),
          progress.data || t("components.asrPanel.loadingModelDefault")
        );
      }
      ...
      if (progress.status === "running") {
        showInfo(
          t("components.asrPanel.processingAudio"),
          t("components.asrPanel.processingAudioDetail")
        );
      }
      ...
      if (progress.status === "loaded") {
        showSuccess(
          t("components.asrPanel.modelLoadSuccess"),
          t("components.asrPanel.modelLoadSuccessDetail")
        );
      }
    };
  ...
  const loadModel = useCallback(async () => {
    try {
      ...
      showSuccess(
        t("components.asrPanel.modelLoadSuccess"),
        t("components.asrPanel.modelLoadReady")
      );
    } catch (error) {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : t("components.asrPanel.modelLoadFailed");
  ...
  const startTranscription = useCallback(
    async (audioBuffer: ArrayBuffer) => {
      if (!videoFile) {
        ...
        setError(errorMsg);
        showWarning(
          t("components.asrPanel.cannotStartTranscription"),
          errorMsg
        );
        return;
      }
      ...
      try {
        setLoading(true);
        showInfo(
          t("components.asrPanel.startASRMessage"),
          t("components.asrPanel.preparingAudio")
        );
        ...
        if (!asrService.isReady()) {
          setASRProgress({
            status: "loading",
            data: t("components.asrPanel.preparingModel"),
          });
          showInfo(
            t("components.asrPanel.preparingModel"),
            t("components.asrPanel.preparingModelDetail")
          );
        ...
        setASRProgress({
          status: "loading",
          data: t("components.asrPanel.startTranscription"),
        });
        showInfo(
          t("components.asrPanel.startASRMessage"),
          t("components.asrPanel.recognizingLanguage", { language })
        );

        await asrService.transcribeAudio(audioBuffer, language);
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : t("components.asrPanel.transcriptionFailed");
        setError(errorMessage);
        showError(
          t("components.asrPanel.transcriptionProcessFailed"),
          errorMessage
        );
      } finally {
        setLoading(false);
  ...
  const retryTranscription = useCallback(
    async (audioBuffer: ArrayBuffer) => {
      ...
      setASRProgress({
        status: "loading",
        data: t("components.asrPanel.retryTranscription"),
      });
      showInfo(
        t("components.asrPanel.retryTranscriptionMessage"),
        t("components.asrPanel.retryTranscriptionDetail")
      );
  ...
  const changeDevice = useCallback(
    (device: "webgpu" | "wasm") => {
      ...
      const deviceName =
        device === "webgpu"
          ? t("components.asrPanel.webgpuName")
          : t("components.asrPanel.wasmName");
      showInfo(
        t("components.asrPanel.deviceSwitchSuccess"),
        t("components.asrPanel.deviceSwitchedTo", { device: deviceName })
      );
    },
  ...
  const changeLanguage = useCallback(
    (newLanguage: string) => {
      ...
      showInfo(
        t("components.asrPanel.languageSwitchSuccess"),
        t("components.asrPanel.languageSwitchedTo", { language: newLanguage })
      );
    },
  ...
  const prepareAudioData = useCallback(async () => {
    if (!videoFile) {
      showWarning(
        t("components.asrPanel.missingVideoFile"),
        t("components.asrPanel.selectVideoFirst")
      );
      return null;
    }
    ...
    try {
      showInfo(
        t("components.asrPanel.preparingAudioData"),
        t("components.asrPanel.extractingAudioFromVideo")
      );
  ...
  const handleStartASR = useCallback(async () => {
    ...
    if (!audioBuffer) {
      showError(
        t("components.asrPanel.cannotStartProcessing"),
        t("components.asrPanel.audioDataPreparationFailed")
      );
      return;
    }
    ...
    if (!isReady()) {
      showInfo(
        t("components.asrPanel.preparingModel"),
        t("components.asrPanel.loadingASRModel")
      );
      await loadModel();
    }
  ...
  const getSimpleStatus = () => {
    if (error) {
      return {
        ...
        text: t("components.asrPanel.statusFailed"),
        color: "text-red-600",
      };
    }
    if (asrProgress?.status === "complete") {
      return {
        ...
        text: t("components.asrPanel.statusCompleted"),
        color: "text-green-600",
      };
    }
    if (
      isLoading ||
      asrProgress?.status === "loading" ||
      asrProgress?.status === "running"
    ) {
      return {
        icon: <Loader2 className="h-4 w-4 text-blue-500 animate-spin" />,
        text: t("components.asrPanel.statusProcessing"),
        color: "text-blue-600",
      };
    }
    if (isReady()) {
      return {
        ...
        text: t("components.asrPanel.statusReady"),
        color: "text-green-600",
      };
    }
    return {
      icon: <Play className="h-4 w-4 text-muted-foreground" />,
      text: t("components.asrPanel.statusPending"),
  ...
  const canRetry =
    error || (asrProgress?.status === "complete" && hasTranscriptChunks);
    ...
    <div className={cn("bg-card border rounded-lg p-6 space-y-4", className)}>
      ...
      {showSettings && (
        <div className="border rounded-lg p-4 bg-muted/30 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <select
                value={deviceType}
                onChange={(e) =>
                  handleDeviceChange(e.target.value as "webgpu" | "wasm")
                }
                className="w-full p-2 border rounded-md bg-background"
                disabled={isLoading}
              >
                <option value="webgpu">
                  {t("components.asrPanel.webgpuRecommended")}
                </option>
                <option value="wasm">
                  {t("components.asrPanel.wasmCompatible")}
                </option>
              </select>
            </div>
          </div>
          ...
          <div className="text-xs text-muted-foreground space-y-1">
            ...
            <p>
              •{" "}
              {t("components.asrPanel.modelDownloadNote", {
                size: deviceType === "webgpu" ? "196MB" : "77MB",
              })}
            </p>
          </div>
        </div>
        ...
        <div className="border rounded-lg p-3 bg-muted/20">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="text-sm font-medium">
              {t("components.asrPanel.recognitionLanguage")}
            </span>
          </div>
          <ASRLanguageSelector
            language={language}
      ...
      {videoFile && (
        <div className="text-xs text-muted-foreground border-t pt-4">
          <p>
            {t("components.asrPanel.fileLabel")}: {videoFile.name}
          </p>
          <p>
            {t("components.asrPanel.typeLabel")}: {videoFile.type}
          </p>
          {videoFile.duration > 0 && (
            <p>
              {t("components.asrPanel.durationLabel")}:{" "}
