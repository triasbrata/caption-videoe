  ...
  (
    ...
    const newTimelineTime = useMemo(() => {
      ...
      for (const segment of keptSegments) {
        if (
          localCurrentTime >= segment.start &&
          localCurrentTime <= segment.end
        ) {
          // 当前时间在这个保留片段内
    ...
    const skip = useCallback(
      (seconds: number) => {
        ...
        const newTime = Math.max(
          0,
          Math.min(duration, localCurrentTime + seconds)
        );
        seekTo(newTime);
      },
    ...
    const handleProgressMouseUp = useCallback(() => {
      ...
      if (videoRef.current) {
        ...
        // 通知外部组件时间更新
        const notifyTime =
          previewMode && keptSegments.length > 0 ? dragTime || 0 : finalTime;
        onTimeUpdate?.(notifyTime);
      }
    ...
    if (!videoUrl) {
      return (
        <div
          className={cn(
            "bg-muted rounded-lg flex items-center justify-center p-12",
            className
          )}
        >
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              {t("components.videoPlayer.uploadVideoFirst")}
            </p>
          </div>
        </div>
      );
    ...
    return (
      <div
        className={cn(
          "bg-card rounded-lg overflow-hidden h-full flex flex-col",
          className
        )}
      >
        {/* 视频区域 */}
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm text-muted-foreground">
              ...
              <span>
                {formatTime(previewMode ? newTimelineDuration : duration)}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                title={
                  previewMode
                    ? t("components.videoPlayer.exitPreviewMode")
                    : t("components.videoPlayer.enterPreviewMode")
                }
              >
                {previewMode ? (
          ...
          {previewMode && keptSegments.length > 0 && (
            <div className="text-xs text-muted-foreground bg-muted/50 rounded p-2">
              <div className="flex justify-between">
                <span>
                  {t("components.videoPlayer.previewModeDescription")}
                </span>
                <span>
                  {t("components.videoPlayer.timeSaved", {
                    time: formatTime(duration - newTimelineDuration),
                  })}
                  (
                  {t("components.videoPlayer.retained", {
                    percentage: (
                      (newTimelineDuration / duration) *
                      100
                    ).toFixed(1),
                  })}
                  )
