...
export function useHotkeys(options: UseHotkeysOptions = {}) {
  const { enableHistoryHotkeys = true, enableGlobalHotkeys = false } = options;
  ...
  useEffect(() => {
    ...
    hotkeys("ctrl+z,cmd+z", { scope }, (event) => {
      ...
      if (canUndo) {
        ...
        showInfo(
          t("hooks.hotkeys.cannotUndo"),
          t("hooks.hotkeys.cannotUndoDetail")
        );
      }
    });
    ...
    hotkeys("ctrl+y,cmd+shift+z,ctrl+shift+z", { scope }, (event) => {
      ...
      if (canRedo) {
        ...
        showInfo(
          t("hooks.hotkeys.cannotRedo"),
          t("hooks.hotkeys.cannotRedoDetail")
        );
      }
    });
  ...
  return {
    shortcuts: {
      ...
      redo: {
        keys: navigator.platform.includes("Mac")
          ? ["⌘", "⇧", "Z"]
          : ["Ctrl", "Y"],
