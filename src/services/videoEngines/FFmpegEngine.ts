...
export class FFmpegEngine implements IVideoProcessingEngine {
    ...
    try {
      ...
      if (!supportsSharedArrayBuffer) {
        return {
          supported: false,
          reason:
            "Browser does not support SharedArrayBuffer; ensure HTTPS and cross-origin isolation are enabled",
          formats: [],
          features: {
            trimming: false,
            concatenation: false,
  ...
  async initialize(
    ...
    try {
      ...
      this.reportProgress(
        "initializing",
        100,
        "FFmpeg engine initialization complete"
      );
      ...
      throw new Error(
        `FFmpeg engine initialization failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  private async loadFFmpegLibrary(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () =>
        reject(new Error("Unable to load FFmpeg.wasm library"));
