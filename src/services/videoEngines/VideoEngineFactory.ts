...
export class VideoEngineFactory {
  ...
  ): Promise<IVideoProcessingEngine> {
   ...
   * 根据用户环境自动选择最适合的引擎
   */
  static async getBestAvailableEngine(
    preferredOrder: VideoEngineType[] = ["webav", "ffmpeg"]
  ): Promise<{
   ...
   * 检查所有引擎的可用性
   */
  static async checkAllEngines(): Promise<
    Record<VideoEngineType, EngineCapabilities>
  > {
  ...
  static registerEngine(
    type: VideoEngineType,
    engineClass: new () => IVideoProcessingEngine
  ) {
