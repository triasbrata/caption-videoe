...
export function debugVideoSegments(
  ...
  segments.forEach((segment, i) => {
    ...
    console.log(`  Segment ${i + 1}: ${segment.start}s - ${segment.end}s (${duration.toFixed(2)}s) ${status}`);
  });
  ...
  console.log('\n📊 Statistics:');
  console.log(`  Original total duration: ${totalOriginalDuration.toFixed(2)}s`);
  console.log(`  Kept duration: ${totalKeptDuration.toFixed(2)}s`);
  console.log(`  Deleted duration: ${deletedDuration.toFixed(2)}s`);
  console.log(`  Compression ratio: ${((totalKeptDuration / totalOriginalDuration) * 100).toFixed(1)}%`);
